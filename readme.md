...
> **📝 Note**: This documentation was generated by GitHub Copilot. All steps, configurations, and code examples should be thoroughly tested and verified in your specific environment before use in production. Always follow your organization's security policies and best practices.

This tool provides a simple way to transfer files from an AWS S3 bucket to an Azure Blob Storage container. It lists all objects in a specified S3 bucket, downloads them temporarily, and uploads them to an Azure Blob Storage container.
...
- Downloads objects temporarily to the local system
- Uploads each object to Azure Blob Storage
- Automatically overwrites files in Azure if they already exist
- Handles large files with chunked uploads and concurrency
...
|-----------|---------------------|---------------------|-------------|
| S3 Bucket Name | `S3_BUCKET` | `--bucket` | Name of the AWS S3 bucket |
| AWS Region | `AWS_REGION` | `--region` | AWS region (e.g., us-east-1) |
| AWS Access Key ID | `AWS_ACCESS_KEY_ID` | `--access-key` | AWS access key ID |
| AWS Secret Access Key | `AWS_SECRET_ACCESS_KEY` | `--secret-key` | AWS secret access key |
| Azure Blob SAS URL | `AZURE_BLOB_SAS_URL` | `--azure-sas` | Azure Blob Storage container SAS URL |
...
```bash
python reads3_copyazure.py --bucket your-bucket-name --region us-east-1 --access-key YOUR_ACCESS_KEY --secret-key YOUR_SECRET_KEY --azure-sas "https://youraccount.blob.core.windows.net/container?sv=..."
```
...
$env:AZURE_BLOB_SAS_URL = "https://youraccount.blob.core.windows.net/container?sv=..."
...
> **⚠️ SECURITY WARNING**: Using root user access keys is strongly discouraged by AWS and security best practices. Root users have unrestricted access to all AWS services and resources. Use this method only for testing purposes and delete the keys immediately after use.
